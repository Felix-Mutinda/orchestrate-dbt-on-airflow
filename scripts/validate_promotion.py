...
def main():
    ...
    for env, c in configs.items():
        print(f"   {env:10}: warehouse={c.warehouse.type:10} | schema={c.warehouse.schema_name:10} | concurrency={c.max_active_tasks}")
    ...
    assert configs["dev"].max_active_tasks > configs["local"].max_active_tasks, "Prod/Dev must scale concurrency"
    
    print("\nPromotion Proof Passed: Code is environment-agnostic. Config is environment-specific.")
    print("   Moving from dev to prod requires changing exactly ONE environment variable.")
