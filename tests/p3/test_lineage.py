...
def test_manifest_contains_expected_models(manifest):
    ...
    model_names = [
        node["name"] for node in nodes.values() if node["resource_type"] == "model"
    ]
...
def test_feature_mart_has_correct_upstream_lineage(manifest):
    ...
    feature_mart_node = next(
        (
            ...
            if node.get("name") == "feature_pickup_zone_hourly"
            and node.get("resource_type") == "model"
        ),
        None,
    )
    assert feature_mart_node is not None, (
        "feature_pickup_zone_hourly not found in manifest"
    )
...
def test_no_orphaned_models(manifest):
    ...
    for model in models:
        ...
        assert len(depends_on) > 0, (
            f"Model {model['name']} has no upstream dependencies. Possible orphan."
        )
